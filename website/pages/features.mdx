...
-  `CallApi` automatically cancels redundant requests which means (No more race conditions🤩)
...
-  **Automatic Content-Type Setting**: `CallApi` sets the `Content-Type` automatically depending on your body data. Supported data types for this automatic setting include:

   -  Object
   -  Query Strings
   -  FormData

-  **JSON Handling**: If you pass in an object, `CallApi` will set `Content-Type` to `application/json`. It will also `JSON.stringify` your body, so you don't have to do it yourself.
...
`CallApi` also includes a `toQueryString` method that helps you convert objects to query strings, making it easy to use this option.
