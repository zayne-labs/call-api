...
CallApi offers unified error handling through an `error` object, which captures both the HTTP errors (errors coming as a response from the API) and standard JavaScript errors.
...
3. `errorData`: The error data which can be an API response or a standard JavaScript error object.

For `HTTP errors`, the `name` is set to `HTTPError` while the `errorData` property will contain the error response data from the API.

```js
const { data, error } = await callApi("some-url");
```

For the `non-HTTP errors`--TypeError, SyntaxError--, the `name` reflects the specific JavaScript error type (i.e., 'TypeError', 'SyntaxError'). The `errorData` property contains the original JavaScript Error object.

This structure allows you to quickly identify and handle different types of errors during API calls.
...
This implies there are no race conditions by default using `callApi`.

`CallApi` uses an internal request management system to ensure that only the most recent request to a given URL, with the same fetch option, is processed. This helps prevent dreaded race conditions.
...
-  **Race Condition Prevention**: This mechanism eliminates race conditions that can occur when rapid, successive API calls are made, such as when fast typing in a search input or multiple button clicks.
-  **Ideal for React Hooks**: This feature is handy when `callApi` is used within React's `useEffect` hook or similar scenarios where component updates might trigger multiple API calls.
