...
#### **`onResponse({ response, request, options, data })`**
...
```js copy
await callApi("/api", {
	onResponse: ({ request, response, options, data }) => {
		// Log response
...
#### **`onResponseError({ request, options, response, errorData })`**
...
```js
const callAnotherApi = callApi.create({
	onResponseError: ({ response, request, options, errorData }) => {
		// Log error response
