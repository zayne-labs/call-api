...
`onResponse` is invoked when a successful response is received. It provides access to the response, request details and fetch options.
...
`onResponseError` is invoked when an error response or a status code >= 400 is received from the api. It provides access to the response object, request details, and fetch options used.
