...
CallApi is advanced fetch wrapper designed with one-to-one similarity to fetch API in mind, while still being as convenient as possible to use.
This means you can use the familiar Fetch API syntax while it offers additional options for convenience.

It includes multiple features out of the box such as hooks/interceptors, request deduplication, retries, error as value, plugins and more. Works on the browser, node (version 18+), workers, deno and bun.

## Why CallApi?

Fetching data is the probably the common task in most applications. But the inbuilt solution for that (fetch API) is way too minimalistic for majority of everyday use cases.
So much so that I (and many others) have to write a lot of unnecessary code to set it up. Or write some quick and dirty abstraction around it, only for it to end up being too coupled to be reusable.
...
<Cards>
	...
	/>
	<Card
		title="Request Lifecycle Interceptors/Hooks"
		description="Callback functions that enables running various side effects at different points in the request lifecycle"
	/>
