...
// @log: The resolved URL will be: https://api.example.com/users/123/posts
```
...
The `params` option can take in an object where the keys are the dynamic parameters and the values are the corresponding values to replace them with. It can also take in an array where the values are the values to replace the dynamic parameters in the order they appear in the URL path.
...
// @log: The resolved URL for both cases will be: https://api.example.com/users/123/posts/456
```
...
// @log: The resolved URL will be: https://api.example.com/users/123/posts?page=1&limit=10&sort=latest
```
