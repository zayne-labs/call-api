...
Fetch supports [URLSearchParams](https://developer.mozilla.org/en-US/docs/Web/API/URLSearchParams) as a request body out of the box. However this usually requires manual setting with `new URLSearchParams()` and `set()` or `append()`.
