...
**Simple:** Based on the Fetch API and mirrors its interface. Only adds useful features with sensible defaults.

**Type-safe:** Built with TypeScript. Full type support for responses and errors via schemas, validators and manual generics. Automatic type generation via OpenAPI schemas will be available soon. Even non-TypeScript users will still get proper type information.
...
- Lack advanced convenience features like retries, request deduplication, schema validation, content-type detection etc.
