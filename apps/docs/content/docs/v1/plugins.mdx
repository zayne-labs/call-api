...
Sometimes your plugin needs additional configuration options beyond the standard `CallApiPlugin` interface. TypeScript automatically infers the return type of `createExtraOptions` and merges it with the callApi's options type.
...
const uploadProgressPlugin = definePlugin({
	...
	createExtraOptions: pluginOptionsSchema.parse,
...
<AutoTypeTable path="./lib/callapi-options.ts" name="CallApiPlugin" />
